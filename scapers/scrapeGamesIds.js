...
import { runWithTimer, storeData } from '../shared/scrapers.js'

const scrapeGamesIds = async () => {
  const games = await fetchGamePassGames()
  const gameIds = await Promise.all(games.map(fetchGameIds))
  storeData(gameIds, '../logs/result.log')
}
...
const fetchGameIds = async (game) => {
  ...
  try {
    ...
    logger.info(`Scraped game: ${getGameTitle(game)}`, {
